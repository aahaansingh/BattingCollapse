...
st.write('BattingCollapse is a tool that allows you to visualise and retrieve "Smart Stats" for player performance in IPL performance.')
...
st.write("For example, a strike rate of 150 is far less impressive now than it would have been in the first season of the IPL. Smart Stats adjust for this by calculating the ratio of the player's strike rate to the average strike rate of a top 6 batsman over every match played by that player.")
st.write("Both true average and true S/R are calculated relative to other top 6 batsmen, but true bowling S/R and true economy are calculated relative to every other bowler.")
st.write("The formula is (player_stat/peer_stat - 1) * 100, so a player with a true average of 20 would have an average 1.2 times that of their peers.")
